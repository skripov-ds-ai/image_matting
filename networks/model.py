from tensorflow.keras.layers import UpSampling2D, DepthwiseConv2D, ReLU, Conv2D, Convolution2D, Softmax, Add, Activation, Input, Lambda, Concatenate
...
def create_model(opt, metrics, loss, trainable_pretrained=True, input_shape=(224, 224, 3)):
    old_model = MobileNet(
    ...
    original_image = Lambda(
        lambda x: x, name='original_image',
        # trainable=True
    )(old_model.input)
